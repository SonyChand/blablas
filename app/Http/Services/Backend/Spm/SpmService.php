...
class SpmService
{
        ...
        if ($request->ajax()) {
            try {
                ...
                $totalData = Spm::where('tahun_id', session('tahun_spm') ?? 1)
                    ...
                    ->where('puskesmas_id', request()->routeIs('spm.rekapServerside') ? session('rekapPuskesmas', 'all') : Auth()->user()->puskesmas_id)
                    ->count();
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('sub_id', function ($data) use (&$i) {
                        ...
                        <a href="' . route('spm.detail', $data->id) . '" class="btn btn-sm btn-primary">
                    ...
                    ->addColumn('id_sub', function ($data) {
                        return $data->subLayanan->id;
                    })
                    ->addColumn('kode', function ($data) {
                        $kode = '
                        <span class="btn btn-primary">' . $data->subLayanan->kode . '</span>';
                        return $kode;
                    })
    ...
    public function dataTableAnggaran($request)
    {
        if ($request->ajax()) {
            try {
                $totalData = Spm::where('tahun_id', session('tahun_spm') ?? 1)
                    ->whereHas('subLayanan', function ($query) {
                        $query->where('layanan_id', 3);
                    })
                    ->where('puskesmas_id', request()->routeIs('spm.rekapServersideAnggaran') ? session('rekapPuskesmas', 'all') : Auth()->user()->puskesmas_id)
                    ->count();
                $totalFiltered = $totalData;
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('total_dilayani', function ($data) {
                        $total_dilayani = '
                        <div class="text-end">' . number_format($data->total_dilayani, 0, ',', '.') . '</div>';
                        return $total_dilayani;
                    })
                    ->addColumn('januari', function ($data) {
                        $januari = '
                        <div class="text-end">' . number_format($data->terlayani_januari, 0, ',', '.') . '</div>';
                        return $januari;
                    })
                    ->addColumn('februari', function ($data) {
                        $februari = '
                        <div class="text-end">' . number_format($data->terlayani_februari, 0, ',', '.') . '</div>';
                        return $februari;
                    })
                    ->addColumn('maret', function ($data) {
                        $maret = '
                        <div class="text-end">' . number_format($data->terlayani_maret, 0, ',', '.') . '</div>';
                        return $maret;
                    })
                    ->addColumn('april', function ($data) {
                        $april = '
                        <div class="text-end">' . number_format($data->terlayani_april, 0, ',', '.') . '</div>';
                        return $april;
                    })
                    ->addColumn('mei', function ($data) {
                        $mei = '
                        <div class="text-end">' . number_format($data->terlayani_mei, 0, ',', '.') . '</div>';
                        return $mei;
                    })

                    ->addColumn('juni', function ($data) {
                        $juni = '
                        <div class="text-end">' . number_format($data->terlayani_juni, 0, ',', '.') . '</div>';
                        return $juni;
                    })
                    ->addColumn('juli', function ($data) {
                        $juli = '
                        <div class="text-end">' . number_format($data->terlayani_juli, 0, ',', '.') . '</div>';
                        return $juli;
                    })
                    ->addColumn('agustus', function ($data) {
                        $agustus = '
                        <div class="text-end">' . number_format($data->terlayani_agustus, 0, ',', '.') . '</div>';
                        return $agustus;
                    })
                    ->addColumn('september', function ($data) {
                        $september = '
                        <div class="text-end">' . number_format($data->terlayani_september, 0, ',', '.') . '</div>';
                        return $september;
                    })
                    ->addColumn('oktober', function ($data) {
                        $oktober = '
                        <div class="text-end">' . number_format($data->terlayani_oktober, 0, ',', '.') . '</div>';
                        return $oktober;
                    })
                    ->addColumn('november', function ($data) {
                        $november = '
                        <div class="text-end">' . number_format($data->terlayani_november, 0, ',', '.') . '</div>';
                        return $november;
                    })
                    ->addColumn('desember', function ($data) {
                        $desember = '
                        <div class="text-end">' . number_format($data->terlayani_desember, 0, ',', '.') . '</div>';
                        return $desember;
                    })
                    ->addColumn('total_terlayani', function ($data) {
                        $total_terlayani = '
                        <div class="text-end">' . number_format($data->total_terlayani, 0, ',', '.') . '</div>';
                        return $total_terlayani;
                    })
                    ->addColumn('belum_terlayani', function ($data) {
                        $belum_terlayani = '
                        <div class="text-end">' . number_format($data->belum_terlayani, 0, ',', '.') . '</div>';
                        return $belum_terlayani;
                    })
                    ->addColumn('total_pencapaian', function ($data) {
                        $total_pencapaian = '
                        <div class="text-end">' . round($data->pencapaian, 2) . '%</div>';
                        return $total_pencapaian;
                    })
                    ->addColumn('action', function ($data) {
                        $actionBtn = '
    ...
    public function dataTableRekap($request)
    {
        if ($request->ajax()) {
            try {
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('sub_id', function ($data) {
                        ...
                        <div class="text-center">
                        <div class="btn-group mx-1">
                            <a href="' . route('spm.detailRekap', $data->subLayanan->id) . '" class="btn btn-sm btn-primary">
                                <i class="fas fa-eye"></i>
                            </a>
                        <button id="btn-edit" type="button" class="btn btn-sm btn-warning" data-id="' . $data->id . '">
                                <i class="fas fa-edit"></i>
                ...
                ';
                    ...
                    ->rawColumns(['action', 'sub_layanan_id', 'sub_id', 'kode', 'satuan', 'total_dilayani', 'januari', 'februari', 'maret', 'april', 'mei', 'juni', 'juli', 'agustus', 'september', 'oktober', 'november', 'desember', 'total_terlayani', 'belum_terlayani', 'total_pencapaian'])
                    ->with([
                        'recordsTotal' => $totalData,
                        'recordsFiltered' => $totalFiltered,
                        'start' => $start
    ...
    public function dataTableRekapAnggaran($request)
    {
        if ($request->ajax()) {
            try {
                $totalDilayani = Spm::where('tahun_id', session('tahun_spm', 1))
