...
<body>
    ...
    <p class="text-right my-0">Ciamis, {{ \Carbon\Carbon::parse($letter->letter_date)->translatedFormat('d F Y') }}</p>
    ...
    <div style="text-indent: 20px;" class="mt-0 pt-0">
        ...
        <table class="table table-sm table-borderless mb-0" style="line-height: 12px;">
            @if ($letter->event_date_start && $letter->event_date_end && $letter->event_date_start != $letter->event_date_end)
                <tr>
                    ...
                    <td>: {{ \Carbon\Carbon::parse($letter->event_date_start)->translatedFormat('l') }} –
                        {{ \Carbon\Carbon::parse($letter->event_date_end)->translatedFormat('l') }} /
                        {{ \Carbon\Carbon::parse($letter->event_date_start)->translatedFormat('d') }} –
                        {{ \Carbon\Carbon::parse($letter->event_date_end)->translatedFormat('d F Y') }}</td>
                </tr>
            @else
                <tr>
                    <td style="width: 15%;">Hari/Tanggal</td>
            ...
            @endif
            @if ($letter->event_time_start && $letter->event_time_end)
                <tr>
                    <td>Waktu</td>
                    <td>: {{ $letter->event_time_start }} s.d {{ $letter->event_time_end }} WIB</td>
                </tr>
            @endif
        ...
        <p class="text-justify">{!! $letter->letter_closing !!}</p>
    </div>
