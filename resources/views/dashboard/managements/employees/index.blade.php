<x-dash.layout>
    ...
    <div class="mb-9">
        <div id="employeeSummary"
            data-list='{"valueNames":["id", "name", "type", "nip", "unit"],"page":6,"pagination":true}'>
            <div class="row mb-4 gx-6 gy-3 align-items-center">
                <div class="col-auto">
                    <h2 class="mb-0">Data Pegawai<span class="fw-normal text-body-tertiary ms-3"></span></h2>
                </div>
            </div>
                    ...
                    <table class="table fs-9 mb-0 border-top border-translucent">
                        <thead>
                            <tr>
                                ...
                                <th class="sort white-space-nowrap ps-0" scope="col" data-sort="type">Jenis Pegawai
                                </th>
                                ...
                                <th class="sort white-space-nowrap ps-0" scope="col">Pangkat/Golongan</th>
                        ...
                        <tbody class="list" id="employee-list-table-body">
                            ...
                            @foreach ($employees as $row)
                                <tr class="position-static">
                                    ...
                                    <td class="type">{{ $row->employee_type }}</td>
                                    <td class="nip">{{ $row->employee_identification_number }}</td>
                                    <td>{{ $row->birth_place }}, {{ $row->birth_date->format('d/m/Y') }}</td>
                                    <td>{{ $row->rank }}</td>
                                    <td>{{ $row->position }}</td>
                                    <td>{{ $row->education_level }}</td>
                                    <td class="unit">{{ $row->work_unit }}</td>
                                    @canany(['employee-edit', 'employee-delete'])
