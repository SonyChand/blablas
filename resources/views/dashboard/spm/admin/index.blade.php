<x-dash.layout>
    ...
    <div class="card shadow-none border mb-4">
        <div class="card-header p-4 border-bottom bg-body d-print-none">
            <form action="{{ route('dashboard.chartSpm') }}" method="POST" class="needs-validation" novalidate>
                @csrf
                <div class="input-group">
                    <select name="chart_spm" class="form-select" id="chart_spm" required>
                        <option value="" hidden>Pilih Indikator</option>
                        @foreach ($subLayanans as $sublayanan)
                            @php
                                $subLayananUraian = $sublayanan->kode . ' - ' . $sublayanan->uraian;
                            @endphp
                            <option value="{{ $sublayanan->id }}"
                                {{ old('chart_spm', session('chart_spm', 1)) == $sublayanan->id ? 'selected' : '' }}>
                                {{ $subLayananUraian }}
                            </option>
                        @endforeach
                        <option value="all"
                            {{ old('chart_spm', session('chart_spm', 1)) == 'all' ? 'selected' : '' }}>Semua</option>
                    </select>
        ...
        @if (session('chart_spm', 1) == 'all')
            <div class="card-header p-4 border-bottom bg-body" style="text-align: center; font-weight: 900">
                Persentase Pencapaian 12 Indikator SPM Puskesmas di Tahun
                {{ \App\Models\Backend\SPM\Tahun::find(session('tahun_spm', 1))->tahun }}
            </div>
