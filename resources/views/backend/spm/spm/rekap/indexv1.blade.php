    ...
    <div class="mb-9">
        <div id="{{ str_replace(' ', '', $title) }}Data">
            <div class="row justify-content-between mb-4 gx-6 gy-3 align-items-center">
                ...
                <div class="col-auto">
                    <form action="{{ route('spm.tahunspmrekap') }}" method="POST" class="needs-validation" novalidate
                        onsubmit="showLoader()">
                        @csrf
                        <div class="input-group mb-3">
                            <select name="rekapPuskesmas" class="form-select" id="rekapPuskesmas" required>
                                <option value="" hidden>Pilih Puskesmas</option>
                                @foreach ($puskesmases as $puskesmas)
                                    <option value="{{ $puskesmas->id }}"
                                        {{ old('rekapPuskesmas', session('rekapPuskesmas', 'all')) == $puskesmas->id ? 'selected' : '' }}>
                                        {{ $puskesmas->nama }}
                                    </option>
                                @endforeach
                                <option value="all"
                                    {{ old('rekapPuskesmas', session('rekapPuskesmas', 'all')) == 'all' ? 'selected' : '' }}>
                                    Semua (Rekap)</option>
                            </select>
        ...
        <script>
            ...
            function tableYajra2() {
                $('#yajra2').DataTable({
                    ...
                    serverSide: true,
                    responsive: true,
                    ajax: "{{ route('spm.rekapServersideAnggaran') }}",
                    columns: [{
                            data: 'sub_id',
                            name: 'sub_id',
                            orderable: true,
            ...
            $('#yajra').on('click', '.kode-click', function() {
                ...
                if (currentRow.next().hasClass('details-row')) {
                    ...
                    $.ajax({
                        url: `/e-spm/spm/get-sublayanan-rekap/${subLayananId}`, // Adjust this URL to your API endpoint
                        method: 'GET',
